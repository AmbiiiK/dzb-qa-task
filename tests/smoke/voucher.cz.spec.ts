...
test.describe('CZ Voucher purchase — happy path', () => {
  for (const method of projectPaymentMethods.cz) {
...
test.describe('CZ Voucher — unhappy path', () => {
  test('missing required field shows validation error', async ({ page, voucherPage, testOrder }) => {
    ...
    await expect(page.getByText(validationErrors.requiredFieldCz)).toBeVisible();
    await expect(page.getByText(validationErrors.formGeneralError)).toBeVisible();
  });
  ...
  test('unchecked T&C checkboxes block submission', async ({ page, voucherPage, testOrder }) => {
    ...
    await expect(page).toHaveURL(/\/voucher/);
    await expect(page.getByText(validationErrors.requiredCheckboxCz).first()).toBeVisible();
    await expect(page.getByText(validationErrors.formGeneralError)).toBeVisible();
  });
});
